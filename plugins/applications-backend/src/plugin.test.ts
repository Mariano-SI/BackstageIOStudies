...
import { applicationsPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [applicationsPlugin],
    });

    await request(server).get('/api/applications/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/applications/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/applications/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/applications/todos/${createdTodoItem.id}`)
