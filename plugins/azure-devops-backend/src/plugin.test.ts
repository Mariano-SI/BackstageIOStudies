...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [azureDevopsPlugin],
    });

    await request(server).get('/api/azure-devops/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/azure-devops/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/azure-devops/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/azure-devops/todos/${createdTodoItem.id}`)
